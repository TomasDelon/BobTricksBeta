# 00 — Scientific and mathematical foundations of the `CM-first` model (v2)

This file supersedes `docs/00_overview.md` for mathematical rendering.  
From this document onward, inline mathematics is written with `$...$` and display mathematics with `$$...$$`.

## 0. Goal of this document

The purpose of this document is to define, rigorously and progressively, the mathematical objects manipulated by the future locomotion engine of `BobTricksBeta`.
...
**the leg cannot change its bone length.**  
Any reduction of $\|P-A_i\|$ must come from knee flexion.
...
The body behaves approximately like an inverted pendulum during single support, and the costly part of the gait is concentrated in the step-to-step transition [Kuo 2007].
...
Dynamic walking theory shows that step-to-step transition is an energetically essential part of walking [Kuo 2007].
...
- $\beta \in [0,1]$ the support transfer parameter.
...
- running is approximately spring-mass / SLIP-like [Blickhan 1989].
