# 00 — Scientific and mathematical foundations of the `CM-first` model (v4)

This file supersedes `docs/00_overview_v3.md` for mathematical rendering.
...
- no display block contains a line that starts with `=`.
...
$$
H = 1.80\,\mathrm{m}, \qquad L = 0.36\,\mathrm{m}.
$$
...
So the visible foot extends essentially forward from the ankle.
...
$$
h_P^{\mathrm{stand}} = \sqrt{4L^2 - 0.2025L^2}.
$$

$$
h_P^{\mathrm{stand}} = \sqrt{3.7975}\,L.
$$

$$
h_P^{\mathrm{stand}} \approx 1.9487L.
$$
...
$$
s_A = (C-A) \cdot t, \qquad z_A = (C-A) \cdot n.
$$
...
$$
\ell_w^\star = 2L - \varepsilon_w L, \qquad 0 < \varepsilon_w \ll 1.
$$
...
$$
\Phi_w(C,\theta,A) = \|P-A\|^2 - (\ell_w^\star)^2.
$$

The nominal manifold condition is
...
$$
\Phi_w(C,\theta,A) = \|C - 0.75L e_\theta - A\|^2 - (\ell_w^\star)^2.
$$

The nominal manifold condition is again
...
$$
d_t = 0.75L (e_\theta \cdot t), \qquad d_n = 0.75L (e_\theta \cdot n).
$$
...
$$
\xi = s_Q + \lambda \frac{\dot s_A}{\omega_0}, \qquad 0 < \lambda \le 1.
$$
...
$$
u_{\mathrm{tan}} = k_v(v_t^\star - \dot s_A)t + k_\xi(\xi^\star - \xi)t.
$$
...
$$
\rho = \mathrm{clamp}\left(\max\left\{ \frac{d(\xi,\mathcal S)-m_\xi}{\Delta_\xi}, \frac{\|P-A_{\mathrm{stance}}\| - \ell_w^\star}{\Delta_\ell}, \frac{|\dot s_A - v_t^\star|}{\Delta_v} \right\}, 0, 1\right).
$$
...
This means that stronger perturbations naturally allow more knee flexion and therefore a lower CM.
...
$$
u_{\mathrm{run}} = k_r(\ell_0 - \ell)_+\,\hat r - c_r \dot \ell\,\hat r + k_t(v_t^\star - \dot s_A)t.
$$
...
$$
\text{TerrainModel} \rightarrow \text{ContactModel} \rightarrow \text{CMController} \rightarrow \text{FootstepPlanner} \rightarrow \text{SwingGenerator} \rightarrow \text{BodyReconstruction} \rightarrow \text{Render}.
$$
